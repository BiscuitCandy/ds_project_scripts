...
biobert = BioBert(model_path = "../models/biobert_CKG")
...
ref_column_1 = [
    ...
    'hypoxia', 'haemoptysis', 'positive auscultation findings',
    'sore throat', 'respiratory symptoms', 'nasal symptoms', 
    'loss of sense of smell or taste', 'fever', 'low body temprature',
    'shivers', 'chills', 'Myalgia or arthralgia', 'Myalgia or fatigue',
    'fatigue', 'headache', 'Nausea/vomiting', 'Diarrhoea', 
    'abdominal pain', 'gastrointestinal symptoms', 
    'low systolic blood presure', 'high systolic blood presure', 
    'tachycardia', 'palpitations chest tightness'
]
...
ref_column_2 = [
    'cholera', 'influenza', 'influenza virus subtypes A and B',
    'influenza', 'hepatitis A and B', 'invasive menigococcal desiase',
    'Haemophilus influenzae type b', 
    'Diphtheria, Tetanus, Acellular Pertussis, Hepatitis B, Polio',
    'streptococcus', 'pneumoniae', 'meningococcal Group B', 
    'Influenza virus subtypes A and type B', 'West Nile virus', 
...
ref_column_3 = [
    'CRAB/MDRA', 'CRPA', 'CRE ((referent)', 
    'Age in years (unit=10 years)', 'Admitting/reporting facility',
    'Acute care hospital', 'Long-term acute', 
    'Long-term care facilities (referent)', 
    'Indwelling medical device (yes Vs. no)', 'MRSA (yes Vs. no)', 
    'Underlying (chronic) diseases', 'Bedbound/chronic ulcer', 
    'Cancer or malignancy', 'Cardiovascular disease', 
    'Cerebrovascular disease', 'Chronic infectious disease#', 
    'Chronic kidney disease (CKD or ESRD)', 
    'Chronic respiratory disease', 'Diabetes', 
    'Liver and gastrointestinal disease', 'Morbid obesity', 
    'Mental disorder (referent)'
]
...
positives = [
    ...
    },
    {
        "$match": {
            "$or": [
                {"tables.tableData": {"$regex": "moderna", "$options": "i"}},
                {"tables.tableData": {"$regex": "covaxin", "$options": "i"}},
                {"tables.tableData": {"$regex": "pneumonia", "$options": "i"}},
                {"tables.tableData": {"$regex": "respiratory", "$options": "i"}},
                {"tables.tabledata": {"$regex": "alpha variant", "$options": "i"}},
                {"tables.tableData": {"$regex": "gamma variant", "$options": "i"}}
            ]
        }
    },
    {
    ...
    if table:
        ...
        for i in range(len(table[0])) :
            ...
            if not isinstance(emb, int) :
                ...
                similarity_table = similarity_table.append({"paper_ID": document["_id"], "table_ID": document["table_id"], "Attribute": table[0][i], "ref_1_similarity": abs(round(1 - cosine(np.array(emb), np.array(ref_emb_1)), 6)), "ref_2_similarity": abs(round(1 - cosine(np.array(emb), np.array(ref_emb_2)), 6)), "ref_3_similarity": abs(round(1 - cosine(np.array(emb), np.array(ref_emb_3)), 6))}, ignore_index = True)

    if count % 200 == 0 : 
      print(count)
      similarity_table.to_csv("covid_column_similarity.csv")
