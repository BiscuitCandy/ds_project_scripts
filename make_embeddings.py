...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_vector_file', help='Output_Vector_File_Path stores the embeddings vectors for words in output_context_file')
    parser.add_argument('--output_context_file', help='output_metadata_file_path')

    args = parser.parse_args()

    if (not args.model) or (not args.input_file) or (not args.output_context_file) or (not args.output_vector_file) :
        parser.print_help()
        exit(0)
    ...
    for tex in texts :
        ...
        try :
            ...
            for i, j in zip(word_embeddings, tokens):
                    vector = str([tensor.item() for tensor in i])
                    data_writer.writerow([vector, ])
                    meta_writer.writerow([j, text.strip()])
        except: 
            print("An exception occurred at word", j, "in text", text)
