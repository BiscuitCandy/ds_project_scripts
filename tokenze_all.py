...
df = pd.read_csv("~/full_cancerkg.csv", sep = "~")
...
def process_df(df, columns=['data', 'meta_h', 'meta_v'], term_wise=True):
    ...
    for col in columns:
        ...
        [col]=df[col].str.replace(r'-\d+(\.\d+)?', 'NEG', regex=True)
        df[col]=df[col].str.replace(r'0(\.\d+)?', 'SMALLPOS', regex=True)
        df[col]=df[col].str.replace(r'\d+(\.\d+)?', 'FLOAT', regex=True)
        # df[col]=df[col].str.replace(r'(?<![a-z|A-Z])(?<![\d.])[0-9]+(?![\d.])(?![a-z|A-Z])', 'INT', regex=True)
        df[col]=df[col].str.replace(r'\d+(\.\d+)?%', 'PERCENT', regex=True)
...
df = df[["meta_v", "meta_h", "data", "label"]]

df.to_csv("tokenized_data.csv", sep="~")